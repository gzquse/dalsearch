...
def seed_generator():
    with open('wangzhe.html', 'rb') as html_data:
...
class SimpleCrawler(object):
    ...
    def crawl_web(self, max_depth):
        ...
        next_depth = []
        depth = 0
        # 只要当前的crawl 的深度小于最大深度就从list里面拿出url进行爬取
    ...
    def save_to_db(self):
        ...
        with connection.cursor() as cursor:
            for item in self.all_page_info:
                query_sql = """INSERT INTO document_page (url,title,image,description,pagecontent) VALUES ("{}","{}","{}","{}","{}");"""\
                    .format(item[1]['url'], item[1]['title'], item[1]['image'].strip('/'), item[1]['content'][:140],item[1]['content'])
                connection.commit()
                print(query_sql)
